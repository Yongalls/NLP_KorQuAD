...
def train(args, model, tokenizer, val_dataset, val_examples, val_features, test_dataset, test_examples, test_features):
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if use_teacher:
                with torch.no_grad():
                    outputs_teacher = teacher(**inputs_teacher) # inputs copy
                student_logits = outputs[1]
                teacher_logits = Variable(outputs_teacher[1].detach().data, requires_grad=False)
...
def main():
    ...
    if args.start_loss:
        ...
        teacher.start_loss = True
    if args.answer_loss:
        student.answer_loss = True
        teacher.answer_loss = True

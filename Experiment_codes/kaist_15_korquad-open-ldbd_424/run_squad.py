...
# def update_teacher(k_sub, k_add, teacher):
#     # Use the true average until the exponential average is more correct
#     for teacher_param, param_sub, param_add in zip(teacher.parameters(), k_sub, k_add):
#         teacher_param.data.mul_(0).add_(1/5, param_add.data).add_(1, param_sub.data.mul_(-1/5))
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if use_teacher:
                inputs_1 = {
                    ...
                    "start_positions": batch[3].clone(),
                    "end_positions": batch[4].clone(),
                }
                inputs_2 = {
                    "input_ids": batch[0].clone(),
                    "attention_mask": batch[1].clone(),
                ...
                # }
                with torch.no_grad():
                    outputs_1 = torch.unsqueeze(k_list[0](**inputs_1)[1], 0)
                    outputs_2 = torch.unsqueeze(k_list[1](**inputs_2)[1], 0)
                    outputs_3 = torch.unsqueeze(k_list[2](**inputs_3)[1], 0)
                    # outputs_4 = torch.unsqueeze(k_list[3](**inputs_4)[1], 0)
                    # outputs_5 = torch.unsqueeze(k_list[4](**inputs_5)[1], 0)

                    # outputs_teacher = torch.cat((outputs_1, outputs_2, outputs_3, outputs_4, outputs_5),0)
                    outputs_teacher = torch.cat((outputs_1, outputs_2, outputs_3),0)

                    outputs_teacher = torch.mean(outputs_teacher, 0)
                    outputs_teacher = torch.squeeze(outputs_teacher, 0)
                student_logits = outputs[1]
                teacher_logits = Variable(outputs_teacher.detach().data, requires_grad=False)
                loss_fct = torch.nn.MSELoss()
                mse_loss = loss_fct(student_logits, teacher_logits)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if use_teacher:
                    k_list.append(copy.deepcopy(model)) # 고치기
                    del k_list[0]
                else:
                    k_list.append(copy.deepcopy(model))
                    if len(k_list) == args.K:
                        use_teacher = True
...
def main():
    ...
    parser.add_argument(
        ...
        default=5,
        type=int,
        help="Hyper-parameter for self-ensembling",
    )
    parser.add_argument(
        "--data_dir",
        default=None,
    ...
    if args.start_loss:
        student.start_loss = True
    if args.answer_loss:
        student.answer_loss = True
    ...
    ### DO NOT MODIFY THIS BLOCK ###
    if IS_ON_NSML:
        bind_nsml(student, tokenizer, args)
        if args.pause:
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, student, tokenizer, val_dataset, val_examples, val_features)

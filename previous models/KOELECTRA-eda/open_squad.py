...
class SquadProcessor(DataProcessor):
    ...
    def _create_examples(self, input_data, set_type):
        ...
        for entry in tqdm(input_data[:]):
            ...
            augquestion = eda_question(question_text)
            if is_training:
                question_text = augquestion
            ...
            for pi, paragraph in enumerate(entry["paragraphs"]):
                ...
                if answer_text not in context_text:
                    ...
                    is_impossible = False
                augcontext = eda_context(context_text, answer_text)

                if not is_impossible and answer_text not in augcontext:
                    raise AnswerError

                if is_training:
                    context_text = augcontext

...
class SquadProcessor(DataProcessor):
    ...
    def _create_examples(self, input_data, set_type):
        ...
        for entry in tqdm(input_data[:]):
            ...
            # augmentation on question text. Not implemented completely.
            # if is_training:
            #     mix = random.randint(0,1)
            #
            #     dict_q = kkma.pos(question_text)
            #     # print(dict_q)
            #     i_JKS = []
            #     i_JKO = []
            #
            ...
            #             i_JKS.append(i)
            #         if tag == 'JKO': # 명사 + 목적격 조사
            #             i_JKO.append(i)
            #
            #     # i_JKO < i_JKS : 이란의 카자르 왕조를/ 세운것은/ 누구지? --> 세운 것은 누구지? 이란의 카자르 왕조를
            #     # i_JKS < i_JKO : 2016년 5월 4일 ECB는/ 어떤 지폐를/ 발행 중단하기로 했을까? --> 어떤 지폐를 2016년 5월 4일 ECB는 발행 중단하기로 했을까?
            #     if mix == 1 and len(i_JKS) == 1 and len(i_JKO) == 1:
            #         print("question --> JKS: {}, JKO: {}".format(i_JKS, i_JKO))
            #         mix = 1
            #
            #         i_JKS = i_JKS[0]
            #         i_JKO = i_JKO[0]
            ...
            #
            #         if i_JKS < i_JKO:
            #             for i in range(i_JKS+1):
            #                 phrase1 += dict_q[i][0]
            #
            #             for i in range(i_JKS+1, i_JKO+1):
            #                 phrase2 += dict_q[i][0]
            #
            #             for i in range(i_JKO+1,len(dict_q)):
            #                 phrase3 += dict_q[i][0]
            #
            #             print("before: ", question_text)
            #             question_text = phrase2 + phrase1 + phrase3
            #
            #         else:
            #             for i in range(i_JKO+1):
            #                 phrase1 += dict_q[i][0]
            #
            #             for i in range(i_JKO+1, i_JKS+1):
            #                 phrase2 += dict_q[i][0]
            #
            #             for i in range(i_JKS+1,len(dict_q)):
            #                 phrase3 += dict_q[i][0]
            #
            #             print("before: ", question_text)
            #             question_text = phrase2 + phrase3 + phrase1
            #
            #     else:
            #         mix = 0
            #

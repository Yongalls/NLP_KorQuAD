...
from open_squad import squad_convert_examples_to_features_train, squad_convert_examples_to_features_evaluate

'''
ans_len = torch.sum(unanswerable)
unans_len = torch.sum(answerable)
...
def train(args, model, tokenizer, val_dataset, val_examples, val_features):
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            '''
            inputs = {
                "input_ids": torch.cat([batch[0], batch[8]], dim=0),
                "attention_mask": torch.cat([batch[1],batch[9]], dim=0),
                "token_type_ids": torch.cat([batch[2],batch[10]], dim=0),
                "start_positions": torch.cat([batch[3],batch[11]], dim=0),
                "end_positions": torch.cat([batch[4],batch[12]], dim=0),
                "is_mixed": torch.cat([batch[7], batch[15]], dim=0)
            }
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if not args.data_dir and ((evaluate and not args.predict_file) or (not evaluate and not args.train_file)):
            ...
            if evaluate:
                ...
                examples, aug_examples = processor.get_train_examples(args.data_dir, filename=args.train_file)

        print("Starting squad_convert_examples_to_features")
        if evaluate:
            features, dataset = squad_convert_examples_to_features_evaluate(
                examples=examples,
                tokenizer=tokenizer,
            ...
            features, dataset = squad_convert_examples_to_features_train(
                examples=examples,
                aug_examples = aug_examples,
                tokenizer=tokenizer,
                max_seq_length=args.max_seq_length,
                doc_stride=args.doc_stride,
                max_query_length=args.max_query_length,

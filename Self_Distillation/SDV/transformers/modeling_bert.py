...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # print("\n\n !!! self attention part !!! \n\n")
        # print("all_head_size: {}, num_attention_heads: {}, attention_head_size:{}".format(self.all_head_size, self.num_attention_heads, self.attention_head_size))
        ...
        # print("calculating attention_scores: ", query_layer.size(), key_layer.transpose(-1,-2).size(), attention_scores.size())
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in BertModel forward() function)
            # print("attention_scores: {}, attention_mask: {}".format(attention_scores.size(), attention_mask.size()))
        ...
        # print("applying attention probs")
        # print("attention_probs: {}, value_layer: {}, context_layer: {}".format(attention_probs.size(), value_layer.size(), context_layer.size()))

        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        context_layer = context_layer.view(*new_context_layer_shape)
        # print("context layer after concat: ", context_layer.size())
        # print("self attention finished")
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        # print("attention: ", hidden_states.size(), self_outputs[0].size(), attention_output.size())
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        # print(hidden_states.size(), self_attention_outputs[0].size())
        ...
        # print(attention_output.size(), intermediate_output.size())
        # print("output")
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            # print(hidden_states.size(), layer_outputs[0].size())
...
class BertForQuestionAnswering(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.init_weights()
        self.start_weight = 2
        self.start_loss = False
        self.answer_loss = False
        ...
        if self.answer_loss:
            if start_positions is not None and end_positions is not None:
                ...
                if ans_len>0 and unans_len>0:
                    ...
                    start_loss_answerable = loss_fct(start_logits[answerable], start_positions[answerable])
                    start_loss_unanswerable = loss_fct(start_logits[unanswerable], start_positions[unanswerable])

                    end_loss_answerable = loss_fct(end_logits[answerable], end_positions[answerable])
                    end_loss_unanswerable = loss_fct(end_logits[unanswerable], end_positions[unanswerable])
                    if self.start_loss:
                        answerable_loss = (self.start_weight*start_loss_answerable + end_loss_answerable)/3
                        unanswerable_loss = (self.start_weight*start_loss_unanswerable + end_loss_unanswerable)/3
                    else:
                        answerable_loss = (start_loss_answerable + end_loss_answerable)/2
                        unanswerable_loss = (start_loss_unanswerable + end_loss_unanswerable)/2
                ...
                outputs = (total_loss, sequence_output,) + outputs
            ...
            if start_positions is not None and end_positions is not None:
                ...
                total_loss = (self.start_weight*start_loss + end_loss) / 3
                outputs = (total_loss,sequence_output,) + outputs
        else:
            if start_positions is not None and end_positions is not None:
                # If we are on multi-GPU, split add a dimension
